        ...
        <div class="card border-0">
            ...
            <div class="card-body w-100">
                <div class="row align-items-center">
                    ...
                    <p class="text-muted small-0">{{ $post->created_at->diffForHumans()}} </p>
                    <div class="mt-4">
                        <form action="{{ route('comment.store', $post->id) }}" method="post">
                            @csrf 

                            <div class="input-group">
                                <textarea name="comment_body{{ $post->id }}" placeholder="Add a comment here..." rows="1" class="form-control sm">{{ old('comment_body'. $post->id) }}</textarea>
                                <button type="submit" class="btn btn-outline-secondary btn-sm">Post</button>
                            </div>
                            <!-- error -->
                            @error('coment_body' . $post->id)
